...
int main() {
	...
	interleaved_ibo->LoadData(indices, 3 *1);
	...
	interleaved_vbo3->LoadData(interleaved3, 6 * 3);
	...
	interleaved_ibo3->LoadData(indices1, 3 * 1);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		transform3 = glm::translate(glm::mat4(1.0f), glm::vec3(glm::sin(static_cast<float>(thisFrame)), 0.0f,-1.0f));
		...
		glDrawElements(GL_TRIANGLES, interleaved_ibo3->GetElementCount(), (GLenum)interleaved_ibo3->GetElementType(), nullptr);
